 ...
 *
 * The when is exhaustive, so a room added to [SessionRoom] does not compile until it is given a palette.
 */
@Composable
@ReadOnlyComposable
...
fun roomTheme(room: Room, isDark: Boolean): RoomTheme {
    return when (room) {
        is NamedRoom -> neutralRoomTheme(isDark)

        SessionRoom.NARWHAL -> RoomTheme(
        ...
        SessionRoom.UNKNOWN -> neutralRoomTheme(isDark)
    }
}
...
private fun neutralRoomTheme(isDark: Boolean) = RoomTheme(
