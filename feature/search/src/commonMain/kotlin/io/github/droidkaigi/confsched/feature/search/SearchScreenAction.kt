...
sealed interface SearchScreenAction {
    data class ToggleBookmark(val id: TimetableItemId) : SearchScreenAction

    data class ChangeQueryText(val text: String) : SearchScreenAction

    data class ToggleDay(val day: DroidKaigi2026Day) : SearchScreenAction

    data class ToggleCategory(val categoryId: Long) : SearchScreenAction
