...
class DoodleScreenRobot(composeUiTest: ComposeUiTest) : Robot(composeUiTest) {

    private val graph = createGraph<DoodleScreenTestGraph>()
    private var backCount = 0

    fun setupSavedDoodle(doodle: Doodle) {
        graph.doodleSubscriptionKey.set(doodle)
    }
    ...
    fun checkSavedDoodle(doodle: Doodle) {
        assertEquals(doodle, graph.doodleMutationKey.invocations.tryReceive().getOrThrow())
    }
